...
# -------------------------------
    ...
    mae = mean_squared_error(y_test, preds, squared=False)  # RMSE
    from sklearn.metrics import r2_score
    r2 = r2_score(y_test, preds)

    st.subheader("📊 Evaluasi Model")
    st.write(f"**MSE (Mean Squared Error):** {mse:.4f}")
    st.write(f"**RMSE (Root Mean Squared Error):** {mae:.4f}")
    ...
    # Interpretasi R2
    if r2 > 0.8:
        interpret += "• Model memiliki **kemampuan sangat baik** dalam menjelaskan variasi konsumsi energi.\n"
    elif r2 > 0.5:
        interpret += "• Model **cukup baik**, mampu menangkap pola utama dalam data.\n"
    else:
        interpret += "• Model **kurang baik**, pola data sulit ditangkap oleh model saat ini.\n"

    # Interpretasi RMSE (MAE)
    if mae < 0.1:
        interpret += "• Error prediksi tergolong **sangat rendah**, hasil prediksi sangat mendekati nilai asli.\n"
    elif mae < 0.2:
        interpret += "• Error prediksi **rendah**, model cukup stabil.\n"
    else:
        interpret += "• Error prediksi **tinggi**, perlu perbaikan fitur atau model.\n"

    # Interpretasi MSE
    if mse < 0.02:
        interpret += "• Model menunjukkan **akurasinya sangat baik** berdasarkan MSE.\n"
    elif mse < 0.05:
        interpret += "• Model memiliki akurasi **cukup baik**, meskipun masih bisa ditingkatkan.\n"
    else:
        interpret += "• Nilai MSE **tinggi**, menandakan prediksi cenderung meleset.\n"

    st.markdown(interpret)

    # -------------------------------
    # Simulasi Prediksi Baru
    # -------------------------------
