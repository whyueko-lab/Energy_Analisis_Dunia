...
# -------------------------------
    ...
    # Interpretasi R2
    if r2 > 0.8:
        interpret_text += "• Model **sangat baik** memahami pola konsumsi energi.\n"
    elif r2 > 0.5:
        interpret_text += "• Model **cukup baik** menangkap pola utama data.\n"
    else:
        interpret_text += "• Model **kurang baik**, variasi data sulit dijelaskan.\n"

    # Interpretasi RMSE
    if rmse < 0.1:
        interpret_text += "• Tingkat kesalahan model **sangat rendah**, prediksi sangat mendekati nilai asli.\n"
    elif rmse < 0.2:
        interpret_text += "• Kesalahan prediksi **rendah**, model cukup stabil.\n"
    else:
        interpret_text += "• Model memiliki **kesalahan tinggi**, perlu peningkatan.\n"

    # Interpretasi MAE
    if mae < 0.1:
        interpret_text += "• MAE menunjukkan model **sangat presisi**.\n"
    elif mae < 0.2:
        interpret_text += "• MAE menunjukkan tingkat presisi **cukup baik**.\n"
    else:
        interpret_text += "• MAE tinggi, prediksi sering menyimpang.\n"

    st.markdown(interpret_text)
